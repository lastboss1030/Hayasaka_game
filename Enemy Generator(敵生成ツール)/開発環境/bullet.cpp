...
void CBullet::HitParticle(int nNum)
{
	...
	if (nNum == 0)
	{
		if (nCntEffectEnemy % 1 == 0)
		{
			for (int nCntEffect = 0; nCntEffect < 20; nCntEffect++)	// 個数
			{
				...
				float fmove = (float)(rand() % 12);	// 速度

				// パーティクル生成
				CParticle::Create(Pos,												// 座標
					D3DXVECTOR3(sinf(fAngle) * fmove, cosf(fAngle) * fmove, 5),		// 移動量
					D3DXVECTOR3(22, 22, 0),											// サイズ
					D3DXCOLOR(0.3f, 1.0f, 0.2f, 1.0f),								// 色
					50,																// 寿命
					0.90f);															// 慣性
			}
		}
	...
	if (nNum == 1)
	{
		if (nCntEffectEnemy % 1 == 0)
		{
			...
			{
				//角度の設定
				float fAngle = ((float)(rand() % 54 - (54 / 2))) / 100 + D3DX_PI * -0.0f;	// 角度
				float fmove = (float)(rand() % 20);	// 速度
