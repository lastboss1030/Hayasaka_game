...
int CEnemy::m_nIndexEnemy = 0;			// 保存番号
int CEnemy::m_nCounterGame = 0;			// ゲームカウンター
int CEnemy::m_nCountObjective = 0;		// 目的のカウンター
...
HRESULT CEnemy::Init(D3DXVECTOR3 pos, D3DXVECTOR3 size, D3DXVECTOR3 move, int nLife, int nType)
{
	...
	m_aEnemy[m_nIndexEnemy].nLife = 2;									// タイプ
	m_aEnemy[m_nIndexEnemy].nType = 0;									// ライフ
...
void CEnemy::Update(void)
{
	...
	// 一定の位置まで来たら止める (停止型)
	if (pos.y >= 200 && m_nType == 2 && m_nMoveCnt < 300)
	{
		m_move.y = 0;
...
void CEnemy::ChangeValue(void)
{
	...
	{
		m_nCounterGame = m_nCountObjective;
	}

	// 目的のカウント増やす
	if (pInputKeyboard->GetPress(DIK_RIGHT) == true)
	{
		m_nCountObjective += 10;
	}
...
// 上限下限
//=============================================================================
void CEnemy::LimitValue(void)
{
	// 目的のカウント下限
	if (m_nCountObjective < 0)
	{
		m_nCountObjective = 0;
	}
}
...
// 目的カウンター取得
//=============================================================================
int CEnemy::GetCountObjective(void)
{
	return m_nCountObjective;
}
